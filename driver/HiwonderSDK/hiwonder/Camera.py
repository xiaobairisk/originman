...
#这是一个用Opencv获取usb摄像头的画面的封装库(this is a wrapper library for obtaining video feed from a USB camera using OpenCV)
...
class Camera:
    def __init__(self, resolution=(640, 480)):
        ...
        # 以子线程的形式获取图像(get image in sub-thread format)
    ...
    def camera_task(self): # 获取摄像头画面线程(get the camera frame thread)
        while True:
            try:
                if self.opened and self.cap.isOpened(): # 判断是否开启(determine whether it is opened)
                    ret, frame_tmp = self.cap.read() # 获取画面(get frame)
                    if ret:
                        if self.flip:
                        ...
                        # 如果获取画面失败，则尝试重新打开摄像头(if obtain the video feed fails, then attempt to reopen the camera)
                        self.frame = None
